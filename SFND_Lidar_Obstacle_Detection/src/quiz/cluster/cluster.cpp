...
void cluster_helper(int indices,const std::vector<std::vector<float>> points,std::vector<int>& cluster,std::vector<bool> &Processed,KdTree* tree, float distanceTol){
   
      Processed[indices]=true;
      cluster.push_back(indices);
      std::vector<int> nearest =tree->search(points[indices],3.0);
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol){
    ...
    cluster_helper(i,points,clusters,Processed,tree,distanceTol);
    cluster.push_back(clusters);
    i++;
    }
	return cluster;
